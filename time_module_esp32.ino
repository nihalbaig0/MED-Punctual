...
#define MORNING_LED_PIN 25
#define DAY_LED_PIN 26
#define NIGHT_LED_PIN 27

String getPartOfDay(const RtcDateTime& dt)
{
    int hour = dt.Hour();
    if (hour >= 5 && hour < 12) {
        return "Morning";
...
void setup ()
{
  ...
  pinMode(MORNING_LED_PIN, OUTPUT);
...
void loop ()
{
  RtcDateTime now = Rtc.GetDateTime();
    printDateTime(now);
    Serial.print(" - ");
    String partOfDay = getPartOfDay(now);
    Serial.println(partOfDay);

    if (partOfDay == "Morning") {
        digitalWrite(MORNING_LED_PIN, HIGH);
        delay(500); // Adjust blink rate as needed
        digitalWrite(MORNING_LED_PIN, LOW);
    ...
    // Ensure LEDs are off for a bit to see the blink effect clearly
    delay(500);
